...
export default class MarketingRepository {
  ...
  static async sendTokenByFeedbackRepo(
    data: ISendTokenFeedbackBody,
    token: string
  ) {
    ...
    try {
      ...
      const tx = api.tx.balances.transferKeepAlive(data.address, value);
      const [info, result] = await Promise.all([
        tx.paymentInfo(owner),
        tx.signAndSend(owner, { nonce }),
      ]);
    ...
    try {
      const validQuery = {
        AND: [
          ...
          ...(query.received_type
            ? [{ received_type: query.received_type }]
            : []),
          ...(query.date_start
            ? [{ date: { gte: new Date(query.date_start) } }]
            : []),
          ...(query.date_end
            ? [{ date: { lte: new Date(query.date_end) } }]
            : []),
        ],
      };
