...
export default class WebsocketHeader {
  static handleWebsocket = async (request: any) => {
      try {
          ...
          const mainnetEnv = process.env.MAINNET_WS_PROVIDER_ENDPOINT;
          const testnetEnv = process.env.TESTNET_WS_PROVIDER_ENDPOINT;
          const devnetEnv = process.env.DEVNET_WS_PROVIDER_ENDPOINT;
          const localEnv = process.env.LOCAL_WS_PROVIDER_ENDPOINT;

          const wsEndpoint = process.env.WS_PROVIDER_ENDPOINT as string;
          process.env.WS_PROVIDER_ENDPOINT = websocket;

          if (wsEndpoint === mainnetEnv) {
