...
export const manualController = async (
  ...
  try {
    ...
    if (!query || !query.start || !query.end)
      return reply.badRequest('Missing or invalid query.');
    ...
    if (!job) {
      job = cron.schedule(
        ...
        async () => {
          ...
          account = Array.from(new Set(account));
          if (account.length > 0) {
            const result = await MarketingRepository.sendTokenRepo(
              account,
              token
            );
...
export const pauseController = async (
  ...
  try {
    ...
    if (job && isJobRunning) {
      ...
      reply.send({
        message: 'Scheduled job is not running or already paused.',
      });
    }
  } catch (error: any) {
    reply.status(500).send('Internal Server Error: ' + error);
...
export const marketingWalletController = async (
  ...
  try {
    ...
    if (!query || !query.page || !query.entry) {
      return reply.badRequest(
        "Missing or invalid required query: 'page', 'entry'."
      );
    }
...
export const marketingFeedbackController = async (
  ...
  try {
    ...
    const token = (request.headers.authorization as string).slice(7);
    const result = await MarketingRepository.sendTokenByFeedbackRepo(
      body,
      token
    );
    if (result instanceof Error) throw result;
